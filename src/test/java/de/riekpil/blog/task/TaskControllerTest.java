...
class TaskControllerTest {
	...
	public void shouldRejectCreatingReviewsWhenUserIsAnonymous() throws Exception {
		...
		this.mockMvc
				.perform(post("/api/tasks/demo").contentType(MediaType.APPLICATION_JSON)
						.content("{\"taskTitle\": \"Learn MockMvc\"}").with(csrf()))
	...
	public void shouldRejectCreatingReviewsWhenUserIsOk() throws Exception {
		 ...
		 * je valorise le comportement de l'appel de la méthode createTask du service
		 * taskService
		 */
	...
	/**
	 * test de la suppression On doit être identifié :@WithMockUser("duke") mais
	 * aussi avoir le rôle ADMIN :
	 * 
	...
	public void shouldRejectDeletingReviewsWhenUserLacksAdminRole() throws Exception {
		this.mockMvc.perform(delete("/api/tasks/demo/42")) // Appel delete http : API REST
				.andExpect(status().isForbidden());
		// Normalement on doit être refusé : 403 car on n'a pas positionné le rôle
	}

	/**
	 * Delete avec le user et le rôle
	 * 
	 * @throws Exception
	 */
	@Test
	public void shouldAllowDeletingReviewsWhenUserIsAdmin() throws Exception {
		this.mockMvc.perform(delete("/api/tasks/demo/42") // Suppression de la tâche 42
				.with(SecurityMockMvcRequestPostProcessors.user("gigi").roles("ADMIN"))
				// le user et ses rôles ne pas oublier @EnableGlobalMethodSecurity(jsr250Enabled
				// = true)
				.with(csrf())
